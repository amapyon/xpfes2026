#!/usr/bin/env python3
"""02_rotary_cursor_size用のWindows/macOS共通ホストアプリ。"""
...
def decode_delta(data: list[int]) -> int:
    """共通HIDデータ[delta, sequence]の先頭バイトを復号する。"""
    if not data:
        return 0
    # このデバイスはReport IDを使わないため、hidapiの先頭バイトがdeltaになる。
    # 2バイト目は診断用の連番なので、カーソル操作では使用しない。
    raw = int(data[0]) & 0xFF
...
class WindowsCursorBackend:
    ...
    def state_path(self) -> Path:
        ...
        return devkit_root() / ".state" / "cursor-size-before.json"
...
class MacCursorBackend:
    ...
    def state_path(self) -> Path:
        ...
        return (
            devkit_root()
            / ".state"
            / "02_rotary_cursor_size.original-scale"
        )
...
def self_test() -> int:
    ...
    backend = create_backend()
    detail = backend.self_test()
    print(f"Host protocol and {backend.name} {detail}")
    return 0
