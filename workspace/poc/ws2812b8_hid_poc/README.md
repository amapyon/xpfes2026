...
- ホスト側簡易デモ: Windowsで目視確認済み、macOSは未確認
- 参加者向け演習としての採用: 未決定
...
| テスト用VID:PID | `1209:0001` |
| Product | `UIAP WS2812B8 PoC` |

`1209:0001`はpid.codesの共有テスト用識別子であり、世界で一意ではありません。教育目的の試作とワークショップ内テスト専用で、製品、製造、販売、再配布には使用できません。
...
約15秒で次の表示を自動実行し、最後に全消灯します。

1. LED 1～8を、安全側の白`64,64,64`で1灯ずつ順番に点灯
2. 全8灯を赤で点灯し、輝度を`16`、`64`、`128`、`255`の順に変更
3. 緑、青についても同じ4段階の輝度を表示
...
途中で`Ctrl+C`を押した場合も全消灯を試みます。

2026-08-16にWindows 11 x64で、8灯の個別制御と赤・緑・青それぞれの
4段階の輝度変化を人が目視確認済みです。macOSでのデモ目視確認は未実施です。
