...
|---|---|---|
| ローカル操作型 | Devkit内のファイルを読み、`workspace/poc/my_device1`を直接編集できる | 生成AIがローカルファイルを作成・修正する |
| Web・手動受け渡し型 | 添付したファイルだけを参照できる | 生成AIから`my_device1.zip`を受け取り、受講者が展開する |
...
| `PROJECT_TEMPLATE/` | `my_device1`の要件、配線、Cプログラム、Makefileのひな型 |
...
1. 生成AIと相談し、作りたいものと完成条件を`workspace/poc/my_device1/REQUIREMENTS.md`へ整理する
2. 使用する部品を`workspace/parts/PARTS_FOR_AI.md`から選ぶ
3. 配線表と安全確認結果を`workspace/poc/my_device1/WIRING.md`へ整理する
4. 電圧、電流、端子、GPIO、ADC、PWM、タイマー、USBとの競合を確認する
5. 安全確認が終わるまで実物を配線しない
6. `workspace/poc/my_device1`のプログラム一式を生成する
7. PC上のDevkitでビルドする
...
- 受講者と生成AIが整理した`workspace/poc/my_device1/REQUIREMENTS.md`
- 安全確認済みの`workspace/poc/my_device1/WIRING.md`と、必要に応じて配線図画像
- `workspace/ai/PROJECT_TEMPLATE/`
- `workspace/ai/PROGRAM_GENERATION_PROMPT.md`

新規作成時は`PROJECT_TEMPLATE`の内容を`workspace/poc/my_device1`の起点として使用します。Web経路で添付するファイルは`WEB_UPLOAD_CHECKLIST.md`、受け取った成果物は`MY_DEVICE_ZIP_CHECKLIST.md`で確認します。エラーを生成AIへ返すときは`BUILD_ERROR_TEMPLATE.txt`をコピーして記入します。
...
- 編集範囲を`workspace/poc/my_device1`に限定する
...
```text
workspace/poc/my_device1/README.md
workspace/poc/my_device1/Makefile
```

次のような二重ディレクトリにしません。

```text
workspace/poc/my_device1/my_device1/README.md
```

### 4. PC上のDevkitで確認する

Web上の生成AIは、受講者のPC上でビルド、書き込み、実機確認を完了したとは扱いません。Devkitの起動ファイルから環境を開始し、`workspace/poc/my_device1`へ移動して、生成されたREADMEの手順に従います。
...
- `my_device1/REQUIREMENTS.md`
- `my_device1/WIRING.md`
- 修正依頼

修正時も、変更箇所だけではなく修正版`my_device1.zip`一式を生成させ、旧版と混在させません。

## 共通の完成条件

- 成果物が`workspace/poc/my_device1`にある
- `REQUIREMENTS.md`と`WIRING.md`が`my_device1`に含まれる
