...
| `01_macro_keyboard` | 共通5ピン配線のKEYをD5 / PC3で使用 |
| `02_rotary_cursor_size` | 共通5ピン配線のS1をD8 / PC6、S2をD9 / PC7で使用 |

両演習では、[指定のセンタースイッチ付きロータリーエンコーダーモジュール](https://electronicwork.shop/items/64b9e54b9dd503007bc60458)を1個使用し、次の5本を共通配線とする。
...
販売ページの仕様は動作電圧5V、1周20パルスである。配線変更前にはUSBを外す。旧3端子エンコーダーのA/C/B配線や、独立したモメンタリスイッチの配線を新モジュールへ流用しない。
...
`01_macro_keyboard`と`02_rotary_cursor_size`で共通使用する例:
...
- GND→GND、S1→D8 / PC6、S2→D9 / PC7、KEY→D5 / PC3、5V→5V
- 内部プルアップ
- 押下時Low
- デバウンスが有効
- キー押下レポート後に解放レポートを送信
...
KEYは押し込み時Lowとする。新モジュールでWindowsとmacOSの実機動作を確認済みである。
...
- Caps LockがOFFか
- モジュールのKEYがD5、GNDがGND、5Vが5Vへ接続されているか
