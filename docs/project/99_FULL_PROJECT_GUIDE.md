...
- どちらの経路も、要件、配線、安全確認、`workspace/my/device1`の成果物仕様、ローカルでのビルド・書き込み・実機確認を共通とする
- 必須演習の標準配線は講師側で提示するが、その後の自由制作におけるUIAPduinoのピン割り当ては講師側で一元管理しない
- 自由制作の配線は、使用ピン、入出力方向、電圧、電流、競合、安全性を制作ごとに確認する
- 受講者の自由制作用プログラムは`workspace/my/device1`、`device2`、`device3`の最大3件に限定する
- 多くの受講者は1件の完成を目標とし、ワークショップ資料、標準手順、講師の通常サポートは`device1`だけを対象とする
- `device2`と`device3`は、`device1`を完成させた受講者が自力で追加案を試すための任意枠とする
...
必須演習は自由制作に必要な基本操作を学ぶための共通課題であり、最終成果物を指定するものではない。最終成果物は受講者自身が決める。HID UsageとProduct文字列は演習・自由制作ごとに設定し、アプリケーションVID:PIDはワークショップ共通の共有テスト用`1209:0001`へ固定する。
...
`1209:0001`はpid.codesの共有テスト用識別子であり、世界で一意ではない。教育目的の試作とワークショップ内テストだけに使用し、製品、製造、販売、再配布には使用しない。Product文字列は演習ごと、参加者の自由制作ごとに設定する。VIDとPIDを定義または直接埋め込むコードには、この制限を示す警告を記載する。
...
過去のPoCで使用した次の値は履歴上の一時値であり、現行仕様として扱わない。
...
USBアプリケーションは、ワークショップ共通の共有テスト用識別子へ固定する。
...
`1209:0001`は世界で一意ではない。教育目的の試作とワークショップ内テストだけに使用し、製品、製造、販売、再配布には使用しない。VIDとPIDを定義または直接埋め込むファームウェア、PC側プログラム、設定ファイルには、この制限を示す警告コメントを記載する。

Product文字列は演習ごと、参加者の自由制作ごとに設定する。USBアプリケーションを作成する場合、次の値を同一リリース内で一致させる。
...
ブートローダー`1209:B803`は変更せず、通常アプリケーション`1209:0001`と混同しない。

同一VID:PIDの複数台を扱う場合は、USBシリアル番号とUsage Page、Usage、Product文字列、デバイス役割を使用して選択する。Product文字列だけを物理個体の一意識別子として扱わない。
...
PC3の内部プルアップを使用するため、`01`ではモジュールの5V電源を接続しない。`02_rotary_cursor_size`へ進むとき、USBを外してからS1、S2、5Vの3本を追加し、次の合計5本とする。
...
必須5演習の完了後に行う自由制作では、最終成果物を受講者自身が決める。受講者は生成AIと相談し、講師側が用意した部品から使用するものを選ぶ。講師側は、自由制作の統一成果物、共通の部品構成、UIAPduinoの共通ピン割り当てを定めず、参加者ごとの割り当てを一元管理しない。

自由制作ごとに、使用GPIO、入出力方向、ADC・PWM・タイマーなどの機能競合、電源電圧、信号電圧、許容電流、負荷電流、GND、保護回路を確認する。生成AIの提案はそのまま安全確認済みとは扱わず、本規約と対象部品の仕様に照らして確認してから配線する。

現時点で配布を決定している部品は次の9種類とする。
...
参加者1人あたりの配布数量は、タクトスイッチを3個、その他8種類を各1個とする。未確認の型番、予備数、抵抗・駆動部品、追加部品を含む最終BOMは未決定とする。

## 18. 赤外線LED

赤外線LEDは可視光LEDと同様に電流制限が必要であり、GPIOや電源へ抵抗なしで直結しない。型番確定後に順方向電圧、連続順方向電流、パルス電流、デューティ比、極性、放射波長、指向角を確認し、電流制限抵抗と必要な駆動回路を決定する。

赤外線は肉眼で点灯状態を確認できないため、「光って見えない」ことを消灯判定にしない。動作確認には、対応する赤外線センサー、カメラなどの確認手段、または電気的な測定を使用する。参加者向け手順では赤外線LEDを目へ向けないよう明記する。

## 19. 赤外線センサー

「赤外線センサー」という名称だけから、受光素子、リモコン受信モジュール、反射型センサーモジュール、測距センサーのいずれかを推測しない。型番確定後に次を確認する。

- 検出方式と対応波長
- 電源電圧と消費電流
- 出力形式がアナログ、デジタル、パルス、I2Cなどのどれか
- 出力電圧範囲とUIAPduino GPIO・ADCへの適合
- アクティブHigh／Low、必要なプルアップ、応答時間
- 赤外線LEDとの距離、向き、外光の影響

5V出力の可能性があるセンサー出力を、端子定格の確認なしにUIAPduinoへ直結しない。

## 20. OLEDディスプレイ

配布するOLEDはSSD1306搭載、128 × 64、I2C接続の4端子品で、回答上の端子順は左からGND、VDD、SCK、SDAである。次の未確認事項を確認してから配線と実装を確定する。

- 電源電圧、消費電流
- 左右を判断するときの基板の向き
- 信号電圧、I2Cプルアップ電圧、既存プルアップ抵抗
- I2Cアドレス
- UIAPduinoで使用するGPIO、タイマー、USB処理との競合
- フレームバッファ、フォント、通信処理が使用するFlash・RAM

電源投入時は表示を安全な初期状態にし、初期化失敗や通信停止がUSB処理を長時間ブロックしない設計とする。
...
- 最終オンライン`setup`の通し検証
- 別のApple Silicon Mac、別ユーザー、macOS 26以降の対象環境
- Developer ID署名・公証を行わない構成で、正式な個別許可手順を検証
...
- 配布元に残ったビルド生成物がZIPから自動除外され、除外件数が表示される
- Windows予約名がない
- シンボリックリンクへ依存しない

## 16. 通信障害時の縮退動作確認

ワークショップ当日はインターネット接続を必須とするため、ネットワーク切断状態を通常運用またはリリース合格条件とはしない。ただし、通信障害時の代替手順を確認するため、事前`setup`完了済みの環境で可能な範囲の次の動作を確認する。
...
生成AIを利用できない状態では自由制作を通常どおり継続できるとは扱わず、復旧待ち、講師が用意した既知の正常な演習、配線済み見本、完成済みデモへ切り替える。
...
- 外部負荷は電源投入直後に安全状態
- 当日の必須ネットワークと生成AIが利用可能
- 事前`setup`完了条件と通信障害時の代替手順を確認
- 参加者1人あたりの配布数量が、タクトスイッチ3個、その他8種類各1個になっている
- 配布部品9種類の未確認型番と予備数を確認
- Windows版・macOS版の両方に`workspace/parts/PARTS_FOR_AI.md`が収録されている
- `PARTS_FOR_AI.md`の部品ID、名称、型番、分類、機能、端子、電源・信号条件、数量、加工状態、確認済み事項、未確認事項、参考演習・PoC、注意事項を確認
- 未確認の仕様が確認済みとして記載されていない
...
- 赤外線LEDの電流制限、極性、駆動条件、不可視光に関する注意を確認
- 赤外線センサーの検出方式、対応波長、出力形式、出力電圧を確認
- OLEDのコントローラー、解像度、I2C／SPI、アドレス、端子配置、電圧、Flash・RAM要件を確認
- 9種類の配布決定と、自由制作での使用方法・サンプルの検証状態を混同していない
...
現行の通常アプリケーションは共有テスト用VID:PID `1209:0001`へ固定し、Product文字列で演習・自由制作を区別する。`1209:0001`は世界で一意ではなく、教育目的の試作とワークショップ内テスト専用である。過去のPoCで使用した一時的なVID:PID、USBシリアル番号、Devkitバージョンは現行仕様として扱わない。
...
アプリケーション用の共有テストVID:PIDをブートローダーと混同しない。
...
同じ`1209:0001`を使う演習はProduct文字列で区別する。
...
KEYは押し込み時Lowとする。PC3の内部プルアップを使用するため、`01`ではS1、S2、5Vを接続しない。GND/KEYの2本配線で実機動作を確認済みである。
...
- Caps LockがOFFか
- モジュールのKEYがD5、GNDがGNDへ接続され、特にGNDが抜けていないか
...
```text
1209:0001  UIAP Macro Keyboard
1209:0001  UIAP Rotary Cursor
1209:0001  UIAP Vibration Console
1209:0001  UIAP Rotary Haptic
TEST3-001  一時USBシリアル番号
TEST7-001  一時USBシリアル番号
```

`1209:0001`だけで対象を選択せず、Product文字列と必要に応じてUSBシリアル番号も照合する。
...
現行のセンタースイッチ付きロータリーエンコーダーモジュールの配線:
...
- 5V端子をUIAPduinoの5V以外へ接続しない
- 旧3端子エンコーダーのA/C/B配線と混同しない
- `02_rotary_cursor_size`ではKEYを配線したままにするが、ファームウェアは読み取らない
- 配線変更前にUSBを外す
...
同じモジュールを`01_macro_keyboard`でも使用するため、演習を切り替えるときに配線し直す必要はない。
...
赤外線LEDが反応していないように見える場合も、肉眼で見えないことだけを故障の根拠にしない。USBを外して極性、電流制限抵抗、配線、駆動レベルを確認し、対応センサーまたは電気的測定で切り分ける。

赤外線センサーが反応しない場合は、型番、検出方式、電源電圧、出力形式、出力電圧、外光、対象物との距離と向きを確認する。型番が違うモジュールの配線例を流用しない。

OLEDが表示しない場合は、コントローラー、I2C／SPI、端子順、電源電圧、信号電圧、I2Cアドレス、RESET端子を確認する。USB動作まで不安定な場合はOLEDを外し、UIAPduino単体の既知の正常な演習へ戻してから切り分ける。
...
これらは過去の検証履歴であり、現行のアプリケーションVID:PIDではない。現行値は共有テスト用`1209:0001`とする。
...
## 2026-08-11 `01_macro_keyboard`の2本配線実機確認

### 対象

- 部品: `GND / S1 / S2 / KEY / 5V`表記のセンタースイッチ付きロータリーエンコーダーモジュール
- 配線: GND→GND、KEY→D5 / PC3
- 未接続: S1、S2、5V
- ファームウェア: 共通`01_macro_keyboard`

### 利用者実機結果

PC3の内部プルアップを使用し、GNDとKEYの2本だけを接続した状態で、センタースイッチ押下によるHIDキーボード入力が従来どおり動作することを確認した。
...
### `00_onboard_led_blink`3連フラッシュ静的確認

工場出荷時の単発周期点滅と区別するため、現行ファームウェアを150ms点灯・150ms間隔の3連フラッシュと1.5秒休止へ変更した。

- クロスコンパイラ構文検査: 合格
- 150ms点灯、150ms間隔、1.5秒休止、3回の定数検査: 合格
- Windows実機での3連フラッシュ: 極性修正後に利用者確認で合格
- macOS実機での3連フラッシュ: 再確認待ち

### `00_onboard_led_blink` LED極性修正

Windows実機で、1.5秒の休止期間が点灯して見える症状を利用者が確認した。時間処理ではOFF設定後に1.5秒待機していたため、原因は`PC0`をアクティブLowとしていた極性定義である。旧macOS実装と実機症状に合わせ、`High=点灯`、`Low=消灯`へ修正した。

- 修正前症状のWindows実機確認: 1.5秒点灯
- 極性修正後のクロスコンパイラ構文検査: 合格
- 極性定数の回帰検査: 合格
- 修正後のWindows実機3連フラッシュ: 利用者確認で合格
- 修正後のmacOS実機3連フラッシュ: 再確認待ち
...
## 2026-08-17 共有テスト用VID:PID統一の静的検証

通常アプリケーションのVID:PIDを共有テスト用`1209:0001`へ統一し、Product文字列を演習・自由制作ごとに設定する方針を反映した。

確認済み:

- preflightと必須USB演習`01`〜`04`のビルド成功
- 生成した5本のELFすべてにDevice DescriptorのVID:PIDバイト`09 12 01 00`が存在
- 必須演習ホストのProduct文字列照合テスト3件が合格
- ファームウェア11ファイルとホスト11ファイルについて、`1209:0001`とテスト専用警告の自動検査2件が合格
- ワークショップの現行プログラム・README・PoC資料から旧アプリケーションVID:PIDのコード参照が除去済み
- ブートローダー`1209:B803`は変更していない

未確認:

- `1209:0001`へ変更後のWindows実機USB列挙とHID動作
- `1209:0001`へ変更後のmacOS実機USB列挙とHID動作
- 同じVID:PIDを使用する複数ボードのProduct文字列・USBシリアル番号による選択

既存テストの契約を簡略版AIテンプレートの「設計結果」「要確認で停止」「プログラム生成OK」に合わせて更新し、テストスイート28件すべてが合格した。

---

## 2026-08-20 Devkit `0.1.3`リリース前静的検証

自由制作の生成AI向け文書について、アイデア比較、要件整理、配線設計、プログラム生成の4段階を参加者向け資料ZIPに収録した。プログラム生成の停止条件は、`WIRING.md`の`要確認`という表記だけではなく、安全上または物理配線上の未確定事項があるかどうかで判定する。ソフトウェア実装上の選択だけであれば、合理的な初期値を選んで生成を続行する。

確認項目:

- `VERSION`、Windows bootstrap lock、macOS bootstrap lockが`0.1.3`で一致
- 参加者向け文書ZIPが4つの生成AI向けプロンプトを含む
- 安全・物理配線の問題とソフトウェア実装上の未確定事項を区別する自動テスト
- 共通USB識別子`1209:0001`とテスト用警告の自動テスト
- Windowsの非UTF-8標準出力を模擬した、AI補助ツールの日本語メッセージ出力テスト
- Windows版・macOS版の参加者向けZIP生成

初回のGitHub Actionsでは、Windows runnerの標準出力が`cp1252`のため、`new_my_device.py`と`prepare_web_handoff.py`が日本語メッセージの出力時に`UnicodeEncodeError`で停止した。両ツールの標準出力と標準エラーをUTF-8に固定し、テスト側で`cp1252`環境を模擬して回帰検査した。

本節の確認は静的検査とパッケージ生成を対象とする。`0.1.3`でのWindows実機およびmacOS実機の再確認は未実施である。
...
> **現行仕様の読み方:** 2026-07-25以降のWindows xPackネイティブ決定、2026-08-01の配布Devkit最終方針、およびそれ以降の日付付き決定を現行仕様とする。USB識別子は2026-08-17の決定を優先する。これ以前のMSYS2、最終オフライン版、macOS 15対応、当日ネットワーク不要、最終成果物を講師側で決める前提、旧アプリケーションVID:PIDに関する記述は、当時の検証履歴または旧方針として保持する。
...
## 2026-08-13 配布部品3種類の追加

### 決定

- 2026-08-12に決定した配布部品6種類へ、次の3種類を追加する
  - 赤外線LED
  - 赤外線センサー
  - OLEDディスプレイ
- 配布を決定した部品は合計9種類とする
...
- 追加した3種類を含む各部品の型番、参加者1人あたりの数量、予備数
- 最終BOMへさらに部品を追加するか
- 赤外線LEDの電流制限抵抗と駆動回路
- 赤外線センサーの方式、電源・信号仕様、接続方法
- OLEDディスプレイのコントローラー、解像度、I2C／SPI、接続方法

### 配布決定の範囲

- 今回の決定は3種類を参加者へ配布することを確定する
- 型番、電気的仕様、標準配線、サンプルプログラム、実機検証の完了を意味しない
- 型番確定後、ハードウェア規約、参加者向け手順、最終BOM、検証記録を更新する

---

## 2026-08-13 生成AI向け部品情報の配置と形式

### 決定

- 主催者が作成・提示する生成AI向け部品情報を`workspace/parts/PARTS_FOR_AI.md`へ配置する
- 文書は文字情報だけで構成し、画像を必須にしない
...
- `PART-01`、`PART-02`、`PART-03`、`PART-04`、`PART-05`、`PART-07`、`PART-08`は購入先URLを部品情報へ記録する
- `PART-04`はLEDテープを主催者が8個単位へ切り分け、GND、VCC、データ入力の3端子を取り付けて1本配布する
- `PART-04`の注文種類は`WS2812B`とする
- `PART-05`は過去の主催者用PoCと同じ`12085P`、16Ωパッシブブザーとする
- `PART-06`タクトスイッチは2端子品とする
- `PART-07`赤外線LEDは5mm砲弾型、放射波長940nmとする
- `PART-08`赤外線センサーは`VS1838B`で、端子表記は`OUT / GND / VCC`とする
- `PART-09`はSSD1306搭載、128 × 64、I2C接続、4端子品とする
- `PART-09`の回答上の端子順は左から`GND / VDD / SCK / SDA`とし、基板を見る向きは引き続き確認する
- `PART-01`と`PART-02`は必須演習で使用した個体を自由制作でも使用し、追加個体は配布しない
...
## 2026-08-13 自由制作における生成AI利用の二経路

### 決定

- 受講者のPCにはWindows版またはmacOS版のDevkit一式がセットアップ済みであることを前提とする
...
- Webインターフェースしか使えない参加者も、PCに導入済みの同じDevkitで制作と検証を行える
...
## 2026-08-17 ワークショップ用USBテスト識別子の統一

### 決定

- 通常アプリケーションのVIDをワークショップ内で`0x1209`へ固定する
- 通常アプリケーションのPIDをワークショップ内で`0x0001`へ固定する
- `1209:0001`はpid.codesの共有テスト用識別子として扱い、世界で一意の識別子とは扱わない
- 使用範囲を、参加者自身が回路とプログラムを作る教育目的の試作、およびワークショップ内テストに限定する
- 製品、製造、販売、完成機器または完成ファームウェアの再配布には使用しない
- Product文字列は演習ごと、参加者の自由制作ごとに設定する
- VIDとPIDを定義またはUSB Descriptorへ直接埋め込むファームウェア、PC側プログラム、設定ファイルには、共有テスト用であることと利用制限を示す警告を記載する
- ブートローダーの登録済みVID:PID `1209:B803`は変更しない
- 過去に使用した`1209:C003`、`1209:C004`、`1209:C005`、`1209:C006`、`1209:D003`、`1209:D008`、`1209:D009`、`1209:D00A`は検証履歴としてのみ残し、現行コードでは使用しない

### 複数デバイスの識別

- 同じVID:PIDを使用するため、ホスト側はVID:PIDだけでなくProduct文字列も照合する
- 同じ演習の複数ボードを個体識別する場合はUSBシリアル番号も使用する
- Product文字列だけを物理個体の一意識別子として扱わない

### 理由

- 参加者が回路とプログラムを自作する教育・試作環境で、共通のテスト識別子として運用する
- 演習や自由制作ごとにPIDを割り当てる作業をなくし、ファームウェアとホスト側の設定不一致を減らす
- Product文字列を制作物ごとに変えることで、同一VID:PIDでも対象演習を確認できるようにする

### リリースへの影響

- ファームウェア、ホスト側プログラム、テンプレート、生成AI向けプロンプト、README、ビルド規約、トラブルシューティング、リリース検査を同時に更新する
- 識別子変更後に全必須演習のビルド、USB列挙、Product文字列照合、HID動作をWindowsとmacOSで再検証する
- pid.codesのテスト識別子の利用条件をREADMEとコードコメントから削除しない
