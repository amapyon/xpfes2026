...
各部品の型番、参加者1人あたりの数量、予備数、および追加部品を含む最終BOMは未決定とする。すべてを同時に接続することは前提にしない。自由制作で使用する部品は、受講者が生成AIと相談し、講師側が用意した部品から選ぶ。
...
型番、参加者1人あたりの数量、予備数、追加部品を含む最終BOMは未決定とする。
...
型番確定前にSSD1306、SH1106、I2C、SPI、表示解像度、I2Cアドレスを決め打ちしない。型番確定後に次を確認する。

- コントローラー、解像度、電源電圧、消費電流
- I2CまたはSPIのインターフェースと端子配置
- 信号電圧、I2Cプルアップ電圧、既存プルアップ抵抗
- I2CアドレスまたはSPIのCS／DC／RESET端子
...
自由制作で配布部品を案内する場合は、部品名だけで配線させず、実際に配布する型番、基板表記、端子配置、電源電圧、信号形式、必要な抵抗・保護回路を示す。型番未確定の赤外線センサーやOLEDについて、生成AIに一般的なモジュールを仮定させた配線をそのまま使用しない。
