...
Free Wi-Fiは利用可能だが、最終参加者向け開発環境は、ワークショップ当日のインターネット接続を前提にしない。
...
- Windows 11 x64
- macOS Apple Silicon
- macOSの最低バージョンはmacOS Sequoia 15
...
最終オフライン参加者向けZIP、別PC、別ユーザー、macOS版は未確認である。過去のMSYS2構成は検証履歴として残すが、現行の参加者向け標準構成には採用しない。
...
macOSの`02_rotary_cursor_size`は、Python・hidapiではなくIOKit HIDとネイティブarm64ホストを使用する。カーソルサイズ変更には非公開CoreGraphics APIを使用し、test12ではスカラー引数の`float32`／`float64` ABIを実行時に判定する。macOS 26.5.2の利用者実機では想定動作を確認したが、USB切断時復元、別Mac、別ユーザー、最低対応macOS、将来のmacOS更新後の互換性は未確認である。

最終`ch32fun`許可リスト、`rv003usb`入力SHA-256の正式固定、ネットワーク切断状態、別Mac・別ユーザー、macOS最低対応版、最終オフラインZIP、署名・公証方針は未確認または未決定である。現行版は参加者向けリリースではなく、オンライン初期化型の主催者検証版である。
...
`02_rotary_cursor_size`はmacOS 26.5.2のApple Silicon実機で基本動作を確認した。ただし、非公開CoreGraphics APIを使用するため、USB切断時復元、別Mac・別ユーザー、最低対応macOS、将来互換性、公開APIだけで実現する代替方式は引き続き未確認または未決定である。最終オフライン配布も未確認である。
...
- 非公開CoreGraphics APIの正式採用可否
- USB切断時復元
- 最終レビュー済み`ch32fun`許可リスト
- `rv003usb`入力SHA-256の正式固定
- ネットワーク切断状態
- 別のApple Silicon Mac、別ユーザー、最低対応macOS
- 最終オフラインZIP
- Developer ID署名・公証または正式な個別許可方針
...
オンライン検証版では、ダウンロード失敗、SHA-256不一致、途中再実行を確認する。

最終オフライン版では、ネットワーク切断状態でセットアップまたは初期化が完了することを確認する。
...
オンライン・ブートストラップ版だけで成功している場合、最終オフライン版の合格としない。
...
- 別のWindows 11 x64 PC
- 別のmacOS Sequoia 15以降Apple Silicon端末
- 管理者権限のないユーザー
- 日本語ロケール
- ネットワーク切断状態
- 別USBポート
