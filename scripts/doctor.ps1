...
foreach ($name in $exerciseNames) {
    ...
    CheckFile (Join-Path $exercise 'Makefile') "$name dispatcher Makefile"
    if (Test-Path -LiteralPath (Join-Path $exercise 'win') -PathType Container) { Pass "$name Windows source" } else { Fail "$name Windows sourceがありません" }
}
...
if (Test-Path -LiteralPath $python -PathType Leaf) {
    & $python (Join-Path $workspace 'exercises\01_macro_keyboard\win\host\hidcheck.py') --self-test
