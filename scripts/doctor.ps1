...
$exerciseNames = @('00_onboard_led_blink', '01_macro_keyboard', '02_rotary_cursor_size', '03_vibration_motor_console', '04_rotary_cursor_haptic')
foreach ($name in $exerciseNames) {
    ...
    $sourceName = switch ($name) {
        ...
        '03_vibration_motor_console' { 'vibration_motor_console.c' }
        '04_rotary_cursor_haptic' { 'rotary_cursor_size.c' }
    }
...
if (Test-Path -LiteralPath $python -PathType Leaf) {
    ...
    & $python (Join-Path $workspace 'exercises\03_vibration_motor_console\host\motorctl.py') self-test
    if ($LASTEXITCODE -eq 0) { Pass 'Vibration console host self-test' } else { Fail 'Vibration console host self-test' }
    & $python (Join-Path $workspace 'exercises\04_rotary_cursor_haptic\host\cursor_size_host.py') self-test
