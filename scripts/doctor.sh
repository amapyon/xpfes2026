...
for exercise in 00_onboard_led_blink 01_macro_keyboard 02_rotary_cursor_size 03_rotary_cursor_haptic; do
    ...
    case "$exercise" in
        ...
        03_rotary_cursor_haptic) source_name=rotary_cursor_size.c ;;
    esac
    [ -f "$directory/$source_name" ] && ok "$exercise common firmware" || bad "$exercise common firmware is missing"
    if [ "$exercise" != 00_onboard_led_blink ]; then
...
if [ -x "$UIAP_PYTHON" ]; then
    ...
    "$UIAP_PYTHON" "$UIAP_WORKSPACE/exercises/03_rotary_cursor_haptic/host/cursor_size_host.py" self-test >/dev/null 2>&1 && ok 'Rotary haptic host self-test' || bad 'Rotary haptic host self-test'
fi
